...
import { CreateGameRequest, JoinGameRequest, GameAction, WebSocketMessage } from './types';
...
app.post('/api/games', (req, res) => {
  try {
    ...
    if (!gameName || !creatorName) {
      return res.status(400).json({ error: 'Game name and creator name are required' });
    }
...
io.on('connection', (socket: SocketWithPlayer) => {
  ...
  socket.on('game_action', (action: GameAction) => {
    ...
    const game = gameManager.executeAction(socket.gameId, action, socket.playerId);
    if (!game) {
